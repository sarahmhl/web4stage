...
<header class="page-heading">
  <div class="page-heading-block">
    <span class="page-heading-kicker">Détail de l offre</span>
    <h1 class="page-heading-title"><?= htmlspecialchars((string) $offer['title'], ENT_QUOTES) ?></h1>
    <p class="page-heading-subtitle">
      Consultez la mission, l entreprise, les compétences attendues et postulez directement depuis cette fiche.
    </p>
  </div>
</header>
...
<section class="page-layout detail-layout">
          ...
          <?php if (\Core\Auth::checkRole(\Core\Auth::ROLE_ETUDIANT)): ?>
            <form method="post" action="<?= htmlspecialchars(\Core\Url::route('wishlist/toggle'), ENT_QUOTES) ?>">
              ...
              <button type="submit" class="btn <?= !empty($isWishlisted) ? 'btn-primary' : 'btn-outline' ?>">
                <?= !empty($isWishlisted) ? 'Retirer de la wish-list' : 'Ajouter à la wish-list' ?>
              </button>
            </form>
          <?php endif; ?>
      ...
      <section class="detail-section">
        ...
        <div class="detail-meta-grid">
          <div>
            ...
            <strong><?= htmlspecialchars((string) ($offer['company_sector'] ?? 'Non renseigné'), ENT_QUOTES) ?></strong>
          </div>
          <div>
            <span class="detail-label">Contact</span>
            <strong><?= htmlspecialchars((string) ($offer['company_email'] ?? 'Non renseigné'), ENT_QUOTES) ?></strong>
          </div>
          <div>
            <span class="detail-label">Téléphone</span>
            <strong><?= htmlspecialchars((string) ($offer['company_phone'] ?? 'Non renseigné'), ENT_QUOTES) ?></strong>
          </div>
          <div>
            <span class="detail-label">Site</span>
            <strong><?= htmlspecialchars((string) ($offer['company_site'] ?? 'Non renseigné'), ENT_QUOTES) ?></strong>
          </div>
        </div>
  ...
  <aside class="side-card">
    <h2 class="side-card-title">Accès rapides</h2>
  ...
  <section class="section">
    <div class="section-header">
      <div>
        ...
        <p class="section-subtitle">Stages actuellement publiés par <?= htmlspecialchars((string) $offer['company'], ENT_QUOTES) ?>.</p>
      </div>
    </div>
        ...
        <article class="offer-card">
          ...
          <footer class="offer-footer">
            ...
            <div class="offer-actions">
              <a href="<?= htmlspecialchars(\Core\Url::route('offres/detail?id=' . (int) $related['id']), ENT_QUOTES) ?>" class="btn btn-outline">Détails</a>
            </div>
          </footer>
        </article>
      <?php endforeach; ?>
